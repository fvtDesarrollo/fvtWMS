...
Public Class frm_listado_picking
                ...
                If ds.Tables(0).Rows.Count > 0 Then
                    If ds.Tables(0).Rows(Fila)("car_nombre") <> "" Then
                        Do While Fila < ds.Tables(0).Rows.Count
                            ...
                            FechaEmb = CDate(ds.Tables(0).Rows(Fila)("fecha_embarque")).ToString("dd-MM-yyyy")

                            If Mid(FechaEmb, 7, 4) = "1900" Then
                                FechaEmb = "-"
                            End If
    ...
    Private Sub CARGA_GRILLA_FAVATEX_EXPORTAR()
        Dim classPicking As class_picking = New class_picking
        Dim SearchChar As String = "-"
        Dim TestPos As Integer = 0
        Dim Fila As Integer = 0
        ...
        Try
            ...
            If CStr(dtpFechaDespacho.Value.Month).Length = 1 Then
                mesCompromisoDesde = Trim("0" + CStr(dtpFechaDespacho.Value.Month))
            Else
                mesCompromisoDesde = CStr(dtpFechaDespacho.Value.Month)
            End If

            If CStr(dtpFechaDespacho.Value.Day).Length = 1 Then
                diaCompromisoDesde = Trim("0" + CStr(dtpFechaDespacho.Value.Day))
            Else
                diaCompromisoDesde = CStr(dtpFechaDespacho.Value.Day)
            End If

            classPicking.fecha_entrega_desde = CStr(dtpFechaDespacho.Value.Year) + mesCompromisoDesde + diaCompromisoDesde
            ...
            _msg = ""
            Grilla.Rows.Clear()
            ds = classPicking.PICKING_LISTADO_PARA_EXPORTAR(_msg)
            If _msg = "" Then
                If ds.Tables(0).Rows.Count > 0 Then
                    If ds.Tables(0).Rows(Fila)("car_nombre") <> "" Then
                        Do While Fila < ds.Tables(0).Rows.Count
                            ...
                            Grilla.Rows(Fila).DefaultCellStyle.BackColor = Color.LightYellow
                ...
                If ds.Tables(0).Rows.Count > 0 Then
                    If ds.Tables(0).Rows(Fila)("car_nombre") <> "" Then
                        Do While Fila < ds.Tables(0).Rows.Count
                            ...
                            ds.Tables(0).Rows(Fila)("bultos_existentes"),
                            ds.Tables(0).Rows(Fila)("cantidad_despachada"),
                            ds.Tables(0).Rows(Fila)("cantidad_bulto_despachado"),
                            ds.Tables(0).Rows(Fila)("cantidad_embarcado"),
                            ds.Tables(0).Rows(Fila)("cantidad_bulto_embarcado"),
                            ds.Tables(0).Rows(Fila)("observacion"),
                            ds.Tables(0).Rows(Fila)("observacion2"))
    ...
    Private Sub btnExportar_Click(sender As Object, e As EventArgs) Handles btnExportar.Click
        Try
            ...
            Call carga_gilla_exportar_picking()
    ...
    Private Sub carga_gilla_exportar_picking()
        Dim classPicking As class_picking = New class_picking
