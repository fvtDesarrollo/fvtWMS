...
Public Class frm_listado_ordenes_pk_listado
    ...
    Private Sub CARGA_GRILLA()
        ...
        Try
            ...
            If cmbCliente.Text = "" Then
                class_PK.car_codigo = 0
            Else
                class_PK.car_codigo = cmbCliente.SelectedValue
            End If

            If chkFechaDespacho.Checked = True Then
                'desde
                If CStr(txtFechaDespachoDesde.Value.Month).Length = 1 Then
                    mesASDesdeDespacho = Trim("0" + CStr(txtFechaDespachoDesde.Value.Month))
                Else
                    mesASDesdeDespacho = CStr(txtFechaDespachoDesde.Value.Month)
                End If

                If CStr(txtFechaDespachoDesde.Value.Day).Length = 1 Then
                    diaASDesdeDespacho = Trim("0" + CStr(txtFechaDespachoDesde.Value.Day))
                Else
                    diaASDesdeDespacho = CStr(txtFechaDespachoDesde.Value.Day)
                End If

                'Hasta
                If CStr(txtFechaDespachoHasta.Value.Month).Length = 1 Then
                    mesASHastaDespacho = Trim("0" + CStr(txtFechaDespachoHasta.Value.Month))
                Else
                    mesASHastaDespacho = CStr(txtFechaDespachoHasta.Value.Month)
                End If

                If CStr(txtFechaDespachoHasta.Value.Day).Length = 1 Then
                    diaASHastaDespacho = Trim("0" + CStr(txtFechaDespachoHasta.Value.Day))
                Else
                    diaASHastaDespacho = CStr(txtFechaDespachoHasta.Value.Day)
                End If

                class_PK.fecha_entrega_desde = CStr(txtFechaDespachoDesde.Value.Year) + mesASDesdeDespacho + diaASDesdeDespacho
                class_PK.fecha_entrega_hasta = CStr(txtFechaDespachoHasta.Value.Year) + mesASHastaDespacho + diaASHastaDespacho
            Else
                class_PK.fecha_entrega_desde = "19000101"
                class_PK.fecha_entrega_hasta = "20501231"
            End If

            If ChkHoraCita.Checked = True Then
                class_PK.hora_inicio = txtHora.Text
            Else
                class_PK.hora_inicio = "-"
            End If
    ...
    Private Sub chkFechaDespacho_CheckedChanged(sender As Object, e As EventArgs) Handles chkFechaDespacho.CheckedChanged
        txtFechaDespachoDesde.Enabled = chkFechaDespacho.Checked
        txtFechaDespachoHasta.Enabled = chkFechaDespacho.Checked
    End Sub
End Class