...
Public Class frm_oden_para_picking
    ...
    Const COL_GRI_P3_CANTIDAD_EN_PALLET As Integer = 8
    Const COL_GRI_P3_CANTIDAD As Integer = 9
    Const COL_GRI_P3_PROCESO As Integer = 10
    Const COL_GRI_P3_CE As Integer = 11
    Const COL_GRI_P3_PE As Integer = 12
    Const COL_GRI_P3_BOTON As Integer = 13
    Const COL_GRI_P3_FILA As Integer = 14
    Const COL_GRI_P3_PALLET_1 As Integer = 15
    Const COL_GRI_P3_PALLET_2 As Integer = 16
    ...
    Private Sub frm_oden_para_picking_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        dtpCompromisoDesde.Value = GLO_FECHA_SISTEMA 'CDate("22-08-2019")
                ...
                If ds.Tables(0).Rows.Count > 0 Then
                    If ds.Tables(0).Rows(Fila)("pro_codigo") > 0 Then
                        Do While Fila < ds.Tables(0).Rows.Count
                            ...
                            If ds.Tables(0).Rows(Fila)("quitar") = True Then
    ...
    Private Sub CONFIGURA_ACCIONES_SEGUN_ESTADOS()
        Try
            ...
            If _abp_estado = 0 Then 'Sin generar
                ...
                ButtonReprocesar.Enabled = False
    ...
    Private Sub CONFIGURA_COLUMNAS_P3()
        ...
        grillaUbicaciones.Columns(COL_GRI_P3_CANTIDAD_EN_PALLET).AutoSizeMode = DataGridViewAutoSizeColumnMode.DisplayedCells
    ...
    Private Sub btnBuscar_Click(sender As Object, e As EventArgs) Handles btnBuscar.Click
        ...
        Call PINTA_PK_CON_ORDENES_PENDIENTES()
    ...
    Private Sub PINTA_PK_CON_ORDENES_PENDIENTES()
        Dim classPicking As class_picking = New class_picking
        ...
        For Each row As DataGridViewRow In Me.GrillaConsolidado.Rows
            ...
            classPicking.cnn = _cnn
            classPicking.pic_codigo = row.Cells(COL_GRI_COD_PICKING).Value
            ds = classPicking.VALIDA_PK_CON_ORDEN_DE_ASIGNACION(_msg)
            If _msg = "" Then
                If ds.Tables(0).Rows.Count > 0 Then
                    If ds.Tables(0).Rows(0)("Cantidad") > 0 Then
                        Call PINTA_CELDA_MORADO(Fila)
                    End If
                End If
            Else
    ...
    Private Sub PINTA_CELDA_MORADO(ByVal fila As Integer)
        GrillaConsolidado.Item(COL_GRI_P2_SELECCION, fila).Style.BackColor = Color.Purple
        GrillaConsolidado.Item(COL_GRI_P2_SELECCION, fila).Style.ForeColor = Color.White

        GrillaConsolidado.Item(COL_GRI_NUM_PICKING, fila).Style.BackColor = Color.Purple
        GrillaConsolidado.Item(COL_GRI_NUM_PICKING, fila).Style.ForeColor = Color.White

        GrillaConsolidado.Item(COL_GRI_FEC_INGRESO, fila).Style.BackColor = Color.Purple
        GrillaConsolidado.Item(COL_GRI_FEC_INGRESO, fila).Style.ForeColor = Color.White

        GrillaConsolidado.Item(COL_GRI_FEC_COMPROMISO, fila).Style.BackColor = Color.Purple
        GrillaConsolidado.Item(COL_GRI_FEC_COMPROMISO, fila).Style.ForeColor = Color.White

        GrillaConsolidado.Item(COL_GRI_HORA_CITA, fila).Style.BackColor = Color.Purple
        GrillaConsolidado.Item(COL_GRI_HORA_CITA, fila).Style.ForeColor = Color.White

        GrillaConsolidado.Item(COL_GRI_CAR_CODIGO, fila).Style.BackColor = Color.Purple
        GrillaConsolidado.Item(COL_GRI_CAR_CODIGO, fila).Style.ForeColor = Color.White

        GrillaConsolidado.Item(COL_GRI_CAR_NOMBRE, fila).Style.BackColor = Color.Purple
        GrillaConsolidado.Item(COL_GRI_CAR_NOMBRE, fila).Style.ForeColor = Color.White

        GrillaConsolidado.Item(COL_GRI_DESPACHARA, fila).Style.BackColor = Color.Purple
        GrillaConsolidado.Item(COL_GRI_DESPACHARA, fila).Style.ForeColor = Color.White

        GrillaConsolidado.Item(COL_GRI_TOTAL_BULTO, fila).Style.BackColor = Color.Purple
        GrillaConsolidado.Item(COL_GRI_TOTAL_BULTO, fila).Style.ForeColor = Color.White

        GrillaConsolidado.Item(COL_GRI_BOTON, fila).Style.BackColor = Color.Purple
        GrillaConsolidado.Item(COL_GRI_BOTON, fila).Style.ForeColor = Color.White
    ...
    Private Sub btnSeleccionar_Click(sender As Object, e As EventArgs) Handles btnSeleccionar.Click
        ...
        For Each row As DataGridViewRow In Me.GrillaConsolidado.Rows
            If row.Cells(COL_GRI_SELECCION).Value = True Then
                If Fila = 0 Then
                    conCliente = row.Cells(COL_GRI_CAR_NOMBRE).Value
                Else
                    If Trim(conCliente) <> Trim(row.Cells(COL_GRI_CAR_NOMBRE).Value) Then
        ...
        Try
            ...
            If _msg = "" Then
                If ds.Tables(0).Rows.Count > 0 Then
                    If ds.Tables(0).Rows(Fila)("pro_codigointerno") <> "" Then
                        Do While Fila < ds.Tables(0).Rows.Count
                            ...
                            If ds.Tables(0).Rows(Fila)("pro_codigointerno") = "182665" Then
                                codMensaje = codMensaje
                            End If

                            codMensaje = ds.Tables(0).Rows(Fila)("cod_mensaje")

                            Mensaje = ds.Tables(0).Rows(Fila)("mensaje")

                            If ds.Tables(0).Rows(Fila)("cantidad_en_ub_pk") = 0 Then
                                If Mensaje = "STOCK DISPONIBLE" Then
                                    Mensaje = "SE REQUIERE RELLENO DE UBICACIÓN DE PK"
                                    codMensaje = 3
                                End If
                            End If
                            ...
                            If codMensaje = 3 Then
                                Call PINTA_CELDA_ROJO(Fila)
                            ElseIf codMensaje = 1 Then
                                Call PINTA_CELDA_VERDE(Fila)
                            ElseIf codMensaje = 2 Then
                ...
                For Each row As DataGridViewRow In Me.GrillaProductos.Rows
                    ...
                    'If row.Cells(COL_GRI_P2_CODIGO).Value = "182495" Then
                    '    row.Cells(COL_GRI_P2_CODIGO).Value = "182495"
                    'End If

                    classPicking.cnn = _cnn
                    classPicking.pro_codigo = row.Cells(COL_GRI_P2_PRO_CODIGO).Value
                    ds = classPicking.VALIDA_PRODUCTO_EN_PK_ASIGNACION(_msg)
                    If _msg = "" Then
                        If ds.Tables(0).Rows.Count > 0 Then
                            If ds.Tables(0).Rows(0)("resultado") <> "" Then
                                Call PINTA_CELDA_AMARILLO(Fila)
                            End If
                        End If
                    Else
                        MessageBox.Show(_msg, Me.Text, MessageBoxButtons.OK, MessageBoxIcon.Error)
                ...
                'If Mensaje <> "" Then
                '    MessageBox.Show("En la seleccion existe codigos que son parte de otras ordenes de asignacones sin aprobar " _
    ...
    Private Sub PINTA_CELDA_AMARILLO(ByVal fila As Integer)
        GrillaProductos.Item(COL_GRI_P2_SELECCION, fila).Style.BackColor = Color.Yellow
        GrillaProductos.Item(COL_GRI_P2_SELECCION, fila).Style.ForeColor = Color.Black

        GrillaProductos.Item(COL_GRI_P2_CODIGO, fila).Style.BackColor = Color.Yellow
        GrillaProductos.Item(COL_GRI_P2_CODIGO, fila).Style.ForeColor = Color.Black

        GrillaProductos.Item(COL_GRI_P2_NOMBRE, fila).Style.BackColor = Color.Yellow
        GrillaProductos.Item(COL_GRI_P2_NOMBRE, fila).Style.ForeColor = Color.Black

        GrillaProductos.Item(COL_GRI_P2_CANTIDAD, fila).Style.BackColor = Color.Yellow
        GrillaProductos.Item(COL_GRI_P2_CANTIDAD, fila).Style.ForeColor = Color.Black

        GrillaProductos.Item(COL_GRI_P2_CANTIDAD_PICKEADA, fila).Style.BackColor = Color.Yellow
        GrillaProductos.Item(COL_GRI_P2_CANTIDAD_PICKEADA, fila).Style.ForeColor = Color.Black

        GrillaProductos.Item(COL_GRI_P2_CANTIDAD_PENDIENTE, fila).Style.BackColor = Color.Yellow
        GrillaProductos.Item(COL_GRI_P2_CANTIDAD_PENDIENTE, fila).Style.ForeColor = Color.Black

        GrillaProductos.Item(COL_GRI_P2_CANTIDAD_UBICACION_PK, fila).Style.BackColor = Color.Yellow
        GrillaProductos.Item(COL_GRI_P2_CANTIDAD_UBICACION_PK, fila).Style.ForeColor = Color.Black

        GrillaProductos.Item(COL_GRI_P2_CANTIDAD_DISPONIBLE_BODEGA, fila).Style.BackColor = Color.Yellow
        GrillaProductos.Item(COL_GRI_P2_CANTIDAD_DISPONIBLE_BODEGA, fila).Style.ForeColor = Color.Black

        GrillaProductos.Item(COL_GRI_P2_OBSERVACION, fila).Style.BackColor = Color.Yellow
        GrillaProductos.Item(COL_GRI_P2_OBSERVACION, fila).Style.ForeColor = Color.Black

        GrillaProductos.Item(COL_GRI_P2_COD_OBSERVACION, fila).Style.BackColor = Color.Yellow
        GrillaProductos.Item(COL_GRI_P2_COD_OBSERVACION, fila).Style.ForeColor = Color.Black
    End Sub
    ...
    Private Sub ToolStripButton2_Click(sender As Object, e As EventArgs) Handles ToolStripButton2.Click
        ...
        '        classPiking.pro_codigo = row.Cells(COL_GRI_P2_PRO_CODIGO).Value
        '        ds = classPiking.VALIDA_PRODUCTO_EN_PK_ASIGNACION(_msg)
        '        If _msg = "" Then
        '            If ds.Tables(0).Rows.Count > 0 Then
        '                If ds.Tables(0).Rows(0)("resultado") <> "" Then
        '                    Do While Fila < ds.Tables(0).Rows.Count
        '                        If Mensaje = "" Then
        '                            Mensaje = row.Cells(COL_GRI_P2_CODIGO).Value + " " + ds.Tables(0).Rows(Fila)("resultado")
        '                        Else
        '                            Mensaje = Mensaje + vbNewLine + row.Cells(COL_GRI_P2_CODIGO).Value + " " + ds.Tables(0).Rows(Fila)("resultado")
        '                        End If
        '                        Fila = Fila + 1
        '                    Loop
    ...
    Private Sub CARGA_GRILLA_OPERARIO_BODEGA()
        ...
        Try
            ...
            classUsuario.abp_codigo = _dab_codigo
    ...
    Private Function GUARDA_ORDEN_BUSQUEDA_PICKING__DETALLE(ByVal conexion As SqlConnection) As Boolean
        ...
        Try
            For Each row2 As DataGridViewRow In Me.grillaUbicaciones.Rows
                ...
                classPicking.cant_pallet = row2.Cells(COL_GRI_P3_CANTIDAD_EN_PALLET).Value
                ...
                classPicking.quitar = row2.Cells(COL_GRI_P3_PE).Value
                classPicking.pallet_1 = IIf(row2.Cells(COL_GRI_P3_PALLET_1).Value.ToString() = "", "-", row2.Cells(COL_GRI_P3_PALLET_1).Value.ToString)
                classPicking.pallet_2 = IIf(row2.Cells(COL_GRI_P3_PALLET_2).Value.ToString() = "", "-", row2.Cells(COL_GRI_P3_PALLET_2).Value.ToString)
    ...
    Private Function GUARDA_ORDEN_BUSQUEDA_PICKING_DETALLE_ACTUALIZA(ByVal conexion As SqlConnection) As Boolean
        ...
        Try
            For Each row2 As DataGridViewRow In Me.grillaUbicaciones.Rows
                ...
                classPicking.apd_fila = row2.Cells(COL_GRI_P3_FILA).Value
                classPicking.cantidad_confirmada = row2.Cells(COL_GRI_P3_CE).Value
                classPicking.pallet_1 = row2.Cells(COL_GRI_P3_PALLET_1).Value
                classPicking.pallet_2 = row2.Cells(COL_GRI_P3_PALLET_2).Value
                classPicking.quitar = row2.Cells(COL_GRI_P3_PE).Value
    ...
    Private Function PROCESA_ASIGNACION_PARA_PICKING(ByVal conexion As SqlConnection, ByVal codigoEstado As Integer) As Boolean
        ...
        Try
            ...
            ds = Nothing
            For Each row2 As DataGridViewRow In Me.grillaUbicaciones.Rows
                If (row2.Cells(COL_GRI_P3_PE).Value = True) Then
                    If MOVIMIENTO_PALLET_DESDE_PK(conexion, row2.Cells(COL_GRI_P3_PALLET).Value, row2.Cells(COL_GRI_P3_PALLET_1).Value, row2.Cells(COL_GRI_P3_PALLET_2).Value, CantidadMover) = True Then
                        If GUARDA_MOVIMIENTO(conexion, row2.Cells(COL_GRI_P3_PALLET).Value, row2.Cells(COL_GRI_P3_CANTIDAD_EN_PALLET).Value, row2.Cells(COL_GRI_P3_PRO_CODIGO).Value, row2.Cells(COL_GRI_P3_COD_BULTO).Value, row2.Cells(COL_GRI_P3_CANTIDAD_EN_PALLET).Value) = False Then
                            ds = Nothing
                            Exit Function
                        End If
                    Else
                    ...
                    End If
                Else
                    If (row2.Cells(COL_GRI_P3_CANTIDAD).Value > row2.Cells(COL_GRI_P3_CE).Value) And row2.Cells(COL_GRI_P3_PE).Value <> True Then
                        Cantidad = 0
                        Cantidad = (row2.Cells(COL_GRI_P3_CANTIDAD_EN_PALLET).Value - row2.Cells(COL_GRI_P3_CE).Value)
                        If MOVIMIENTO_A_PISO_EXTRAVIADO(conexion, row2.Cells(COL_GRI_P3_PRO_CODIGO).Value, Cantidad, row2.Cells(COL_GRI_P3_COD_BULTO).Value, row2.Cells(COL_GRI_P3_PALLET).Value) = True Then
                            If GUARDA_MOVIMIENTO(conexion, row2.Cells(COL_GRI_P3_PALLET).Value, Cantidad, row2.Cells(COL_GRI_P3_PRO_CODIGO).Value, row2.Cells(COL_GRI_P3_COD_BULTO).Value, Cantidad) = False Then
                                ds = Nothing
                                Exit Function
                            End If
                        Else
    ...
    '=====================================================================================
    ' DESDE PANTALLA DE MOVIMIENTO
    '=====================================================================================
    Private Function MOVIMIENTO_PALLET_DESDE_PK(ByVal conexion As SqlConnection, ByVal _palletRequerido As String, ByVal palletEncontrado1 As String, ByVal palletEncontrado2 As String, ByRef CantidadMover As Integer) As Boolean
        Dim classUbicaciones As class_ubicaciones = New class_ubicaciones
        Dim _msgError As String = ""
        Dim ds As DataSet

        MOVIMIENTO_PALLET_DESDE_PK = False
        ...
        Try
            ...
            ds = classUbicaciones.MOVIMIENTO_PALLET_DESDE_PK(_msgError, conexion)
            If _msgError <> "" Then
                ds = Nothing
                MessageBox.Show(_msgError, Me.Text, MessageBoxButtons.OK, MessageBoxIcon.Error)
                Exit Function
            Else
                If ds.Tables(0).Rows(0)("cantidad_movimiento") < 0 Then
                    MessageBox.Show(ds.Tables(0).Rows(0)("mensaje"), Me.Text, MessageBoxButtons.OK, MessageBoxIcon.Error)
                    ds = Nothing
                    Exit Function
                Else
                    CantidadMover = ds.Tables(0).Rows(0)("cantidad_movimiento")
                    MOVIMIENTO_PALLET_DESDE_PK = True
                End If
            End If
            ...
            If palletEncontrado2 <> "-" Then
                ds = Nothing
                classUbicaciones.cnn = _cnn
                classUbicaciones.palletRequerido = "-"
                classUbicaciones.palletEncontrado = palletEncontrado2
                ds = classUbicaciones.LIBERA_UBICACION_PROCESO_PICKEO(_msgError, conexion)
                If _msgError <> "" Then
                    ds = Nothing
                    MessageBox.Show(_msgError, Me.Text, MessageBoxButtons.OK, MessageBoxIcon.Error)
                    Exit Function
    ...
    Private Sub grillaUbicaciones_CellContentClick(sender As Object, e As DataGridViewCellEventArgs) Handles grillaUbicaciones.CellContentClick
        If e.ColumnIndex = Me.grillaUbicaciones.Columns.Item(COL_GRI_P3_BOTON).Index Then
            ...
            If grillaUbicaciones.Rows(e.RowIndex).Cells(COL_GRI_P3_PE).Value.ToString = "1" Or grillaUbicaciones.Rows(e.RowIndex).Cells(COL_GRI_P3_PE).Value.ToString = "True" Then
                MessageBox.Show("No el pallet fue quitado de la selección", Me.Text, MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
                Exit Sub
            End If
            ...
            frm.cantidadPallet = grillaUbicaciones.Rows(e.RowIndex).Cells(COL_GRI_P3_CANTIDAD).Value
            frm.pro_codigo = grillaUbicaciones.Rows(e.RowIndex).Cells(COL_GRI_P3_PRO_CODIGO).Value
            frm.cod_bulto = grillaUbicaciones.Rows(e.RowIndex).Cells(COL_GRI_P3_COD_BULTO).Value
            frm.ShowDialog()
            ...
            If GLO_ACCION_EJECUTADA = True Then
                ...
                grillaUbicaciones.Rows(e.RowIndex).Cells(COL_GRI_P3_PALLET_1).Value = GLO_PALLET_1
                grillaUbicaciones.Rows(e.RowIndex).Cells(COL_GRI_P3_PALLET_2).Value = GLO_PALLET_2
                GLO_ACCION_EJECUTADA = False
            End If

            If GLO_MUEVE_PALLET_PRODUCTO_EXTRAVIADO = True Then
                grillaUbicaciones.Rows(e.RowIndex).Cells(COL_GRI_P3_PE).Value = 1
            ...
            For Each row As DataGridViewRow In Me.GrillaProductoBulto.Rows
                ...
                For Each row2 As DataGridViewRow In Me.grillaUbicaciones.Rows
                    If row2.Cells(COL_GRI_P3_PRO_CODIGO).Value = varProCodigo And row2.Cells(COL_GRI_P3_COD_BULTO).Value = varBulCodigo Then
                        'Se agrego validacion que excluye las filas marcadas en gris
                        If row2.Cells(COL_GRI_P3_PROCESO).Value = 0 And (row2.Cells(COL_GRI_P3_PE).Value.ToString <> "1" Or row2.Cells(COL_GRI_P3_PE).Value.ToString <> "True") Then
                            varSumaCant = varSumaCant + row2.Cells(COL_GRI_P3_CE).Value
                        End If
                    End If
                Next
                'row.Cells(COL_GRI_P4_ENCONTRADO).Value = IIf(row.Cells(COL_GRI_P4_ENCONTRADO).Value.ToString = "", 0, row.Cells(COL_GRI_P4_ENCONTRADO).Value) + varSumaCant
    ...
    Private Sub PINTA_CELDA_GRIS(ByVal fila As Integer)
        grillaUbicaciones.Item(COL_GRI_P3_CODIGO, fila).Style.BackColor = Color.Gray
        grillaUbicaciones.Item(COL_GRI_P3_CODIGO, fila).Style.ForeColor = Color.White

        grillaUbicaciones.Item(COL_GRI_P3_NOMBRE, fila).Style.BackColor = Color.Gray
        grillaUbicaciones.Item(COL_GRI_P3_NOMBRE, fila).Style.ForeColor = Color.White

        grillaUbicaciones.Item(COL_GRI_P3_COD_BULTO, fila).Style.BackColor = Color.Gray
        grillaUbicaciones.Item(COL_GRI_P3_COD_BULTO, fila).Style.ForeColor = Color.White

        grillaUbicaciones.Item(COL_GRI_P3_BULTO, fila).Style.BackColor = Color.Gray
        grillaUbicaciones.Item(COL_GRI_P3_BULTO, fila).Style.ForeColor = Color.White

        grillaUbicaciones.Item(COL_GRI_P3_COD_UBICACION, fila).Style.BackColor = Color.Gray
        grillaUbicaciones.Item(COL_GRI_P3_COD_UBICACION, fila).Style.ForeColor = Color.White

        grillaUbicaciones.Item(COL_GRI_P3_UBICACION, fila).Style.BackColor = Color.Gray
        grillaUbicaciones.Item(COL_GRI_P3_UBICACION, fila).Style.ForeColor = Color.White

        grillaUbicaciones.Item(COL_GRI_P3_PALLET, fila).Style.BackColor = Color.Gray
        grillaUbicaciones.Item(COL_GRI_P3_PALLET, fila).Style.ForeColor = Color.White

        grillaUbicaciones.Item(COL_GRI_P3_CANTIDAD, fila).Style.BackColor = Color.Gray
        grillaUbicaciones.Item(COL_GRI_P3_CANTIDAD, fila).Style.ForeColor = Color.White

        grillaUbicaciones.Item(COL_GRI_P3_PROCESO, fila).Style.BackColor = Color.Gray
        grillaUbicaciones.Item(COL_GRI_P3_PROCESO, fila).Style.ForeColor = Color.White

        grillaUbicaciones.Item(COL_GRI_P3_CE, fila).Style.BackColor = Color.Gray
        grillaUbicaciones.Item(COL_GRI_P3_CE, fila).Style.ForeColor = Color.White

        grillaUbicaciones.Item(COL_GRI_P3_PE, fila).Style.BackColor = Color.Gray
        grillaUbicaciones.Item(COL_GRI_P3_PE, fila).Style.ForeColor = Color.White

        grillaUbicaciones.Item(COL_GRI_P3_BOTON, fila).Style.BackColor = Color.Gray
        grillaUbicaciones.Item(COL_GRI_P3_BOTON, fila).Style.ForeColor = Color.White

        grillaUbicaciones.Item(COL_GRI_P3_FILA, fila).Style.BackColor = Color.Gray
        grillaUbicaciones.Item(COL_GRI_P3_FILA, fila).Style.ForeColor = Color.White
    End Sub
    ...
    Private Sub GenerarNuevaBusquedaToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles GenerarNuevaBusquedaToolStripMenuItem.Click
        ...
        Dim varProCodigo As Integer = 0
        Dim varBulCodigo As Integer = 0
        Dim ejecuto As Boolean = 0

        Try
            Call NUEVA_BUSQUEDA(ejecuto)
    ...
    Private Function VULEVE_A_PROCESO() As Boolean
        Dim classPicking As class_picking = New class_picking
        Dim _msgError As String = ""
        Dim ds As DataSet
        Dim conexion As New SqlConnection(_cnn)
    ...
    Private Sub NUEVA_BUSQUEDA(ByRef ejecuto As Boolean)
        Dim classPicking As class_picking = New class_picking
        Dim classProductos As class_producto = New class_producto
        Dim Seleccionados As String = ""
        Dim Fila As Integer = 0
        ...
        Dim varProCodigo As Integer = 0
        Dim varBulCodigo As Integer = 0
        Dim _Mensaje As String = "Producto sin stock o cantidad requerida no corresponde a un pallet completo (rellenar zona de PK), para los siguientes codigos:  "
        Dim contador As Integer = 0
        ...
        Try
            ...
            For Each row As DataGridViewRow In Me.GrillaProductoBulto.Rows
                classPicking.cnn = _cnn
                classPicking.pro_codigo = row.Cells(COL_GRI_P4_PRO_CODIGO).Value
                classPicking.bul_codigo = row.Cells(COL_GRI_P4_COD_BULTO).Value
                classPicking.cantidad = (row.Cells(COL_GRI_P4_CANTIDAD).Value - row.Cells(COL_GRI_P4_ENCONTRADO).Value)
                classPicking.abp_codigo = _dab_codigo
                ...
                If _msg = "" Then
                    ...
                    Else
                        contador = contador + 1
                        '_Mensaje =
                        _Mensaje = _Mensaje + Chr(10) + "Codigo: " + row.Cells(COL_GRI_P4_CODIGO).Value + " | Bulto: " + row.Cells(COL_GRI_P4_BULTO).Value + Chr(10)
                    End If
                Else
                    MessageBox.Show(_msg, Me.Text, MessageBoxButtons.OK, MessageBoxIcon.Error)
                    Exit Sub
            ...
            If contador > 0 Then
                ejecuto = False
                MessageBox.Show(_Mensaje, Me.Text, MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
            End If
    ...
    Private Sub btnImprime_Click(sender As Object, e As EventArgs) Handles btnImprime.Click
        ...
        frm.Origen = "OCO"
        frm.adp_codigo = _dab_codigo
        frm.ShowDialog()
    End Sub

    Private Sub BtnReprocesa_Click(sender As Object, e As EventArgs) Handles BtnReprocesa.Click
        'Dim cantidad As Integer = 0
        'Try
        '    For Each row As DataGridViewRow In Me.GrillaProductoBulto.Rows
        '        If (row.Cells(COL_GRI_P4_SELECCION).Value = True) Then
        '            cantidad = cantidad + 1
        '        End If
        '    Next

        '    If cantidad = 0 Then
        '        MessageBox.Show("Debe seleccionar a lo menos una fila", Me.Text, MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
        '        Exit Sub
        '    End If
        ...
        '        If (row.Cells(COL_GRI_P4_SELECCION).Value = True) Then
        '            If row.Cells(COL_GRI_P4_CANTIDAD).Value <= row.Cells(COL_GRI_P4_ENCONTRADO).Value Then
        '                row.Cells(COL_GRI_P4_SELECCION).Value = False
        '            End If
        '        End If
        '    Next
    ...
    Private Sub ButtonReprocesar_Click(sender As Object, e As EventArgs) Handles ButtonReprocesar.Click
        Dim cantidad As Integer = 0
        Dim ejecuto As Boolean = 0
        Try
            For Each row As DataGridViewRow In Me.GrillaProductoBulto.Rows
            ...
            If VULEVE_A_PROCESO() = False Then
                Exit Sub
            End If
    ...
    Private Sub GrillaConsolidado_GridColorChanged(sender As Object, e As EventArgs) Handles GrillaConsolidado.GridColorChanged
