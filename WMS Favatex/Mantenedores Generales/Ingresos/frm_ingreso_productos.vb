...
Public Class frm_ingreso_productos
                ...
                If _msg = "" Then
                    If ds.Tables(0).Rows.Count > 0 Then
                        If ds.Tables(0).Rows(Fila)("car_codigo") > 0 Then
                            Do While Fila < ds.Tables(0).Rows.Count
                                grd_Exclusividad.Rows.Add(ds.Tables(0).Rows(Fila)("car_rut"),
            ...
            If _msg = "" Then
                If ds.Tables(0).Rows.Count > 0 Then
                    If ds.Tables(0).Rows(Fila)("pro_codigo") > 0 Then
                        ...
                        If ds.Tables(0).Rows(Fila)("pro_espack") = True Then
                            chkVentaPack.Checked = True
                            txtCantidadUnidad.Enabled = True
                            txtCantidadUnidad.Text = ds.Tables(0).Rows(Fila)("pro_cantidadpack")
                        Else
                            chkVentaPack.Checked = False
                            txtCantidadUnidad.Enabled = False
                            txtCantidadUnidad.Text = "0"
                        End If
    ...
    Private Function VALIDA_FORM()
        ...
        If chkVentaPack.Checked = True Then
            If txtCantidadUnidad.Text = "" Then
                MessageBox.Show("Debe ingresar la cantidad de unidades por pack, favor verificar", Me.Text, MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
                txtCantidadUnidad.Focus()
                Exit Function
            End If
        End If
    ...
    Private Sub GUARDA_REGISTRO()
        ...
        Try
            ...
            class_producto.pro_espack = chkVentaPack.Checked
            class_producto.pro_cantidadpack = CInt(txtCantidadUnidad.Text)
        ...
        Try
            Using Transaccion As New TransactionScope(TransactionScopeOption.Required, scopeOptions)
                ...
                For Each row As DataGridViewRow In Me.grd_Exclusividad.Rows
                    If CInt(row.Cells(3).Value) > 0 Then
                        class_producto.pro_codigo = _pro_codigo
                        class_producto.car_codigo = row.Cells(2).Value
                        class_producto.cantidad = row.Cells(3).Value
    ...
    Private Sub chkVentaPack_CheckedChanged(sender As Object, e As EventArgs) Handles chkVentaPack.CheckedChanged
        txtCantidadUnidad.Enabled = chkVentaPack.Checked
        If chkVentaPack.Checked = False Then
            txtCantidadUnidad.Text = "0"
        Else
            txtCantidadUnidad.Focus()
        End If
    ...
    Private Sub txtCantidadUnidad_KeyPress(sender As Object, e As KeyPressEventArgs) Handles txtCantidadUnidad.KeyPress
        If Char.IsDigit(e.KeyChar) Then
            e.Handled = False
        ElseIf Char.IsControl(e.KeyChar) Then
            e.Handled = False
    ...
    Private Sub grd_Exclusividad_CellContentClick(sender As Object, e As DataGridViewCellEventArgs) Handles grd_Exclusividad.CellContentClick

    End Sub

    Private Sub grd_Exclusividad_EditingControlShowing(sender As Object, e As DataGridViewEditingControlShowingEventArgs) Handles grd_Exclusividad.EditingControlShowing
        Dim validar As TextBox = CType(e.Control, TextBox)

        cellTextBox = TryCast(e.Control, DataGridViewTextBoxEditingControl)
        PrecionaTeclaDesde = "grd_Exclusividad"
        ' agregar el controlador de eventos para el KeyPress  
        AddHandler validar.KeyPress, AddressOf validar_Keypress_cantidades2
    End Sub

    Private Sub validar_Keypress_cantidades2(
        ByVal sender As Object,
        ByVal e As System.Windows.Forms.KeyPressEventArgs)

        ' obtener indice de la columna  
        Dim columna As Integer = grd_Exclusividad.CurrentCell.ColumnIndex
        Dim fila As Integer = grd_Exclusividad.CurrentCell.RowIndex
