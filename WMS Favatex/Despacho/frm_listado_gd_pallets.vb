...
Public Class frm_listado_gd_pallets
    ...
    Const COL_BTNCIERRE As Integer = 10

    Const COL_SELLO4 As Integer = 0
    Const COL_FDESPACHO4 As Integer = 1
    Const COL_FRETORNO4 As Integer = 2
    Const COL_CLIENTE4 As Integer = 3
    Const COL_MOTIVO_CIERRE4 As Integer = 4
    Const COL_NGUIA4 As Integer = 5
    Const COL_TPALLET4 As Integer = 6
    ...
    Private Sub CARGA_COMBO_CLIENTES_4()
        ...
        Try
            ...
            If _msg = "" Then
                Me.CmbCliente4.DataSource = ds.Tables(0)
                Me.CmbCliente4.DisplayMember = "MENSAJE"
                Me.CmbCliente4.ValueMember = "CODIGO"
            Else
    ...
    Private Sub JUSTIFICA_GRILLA_CIERRE()
        Grilla.Columns(COL_SELLO4).AutoSizeMode = DataGridViewAutoSizeColumnMode.DisplayedCells
        Grilla.Columns(COL_FDESPACHO4).AutoSizeMode = DataGridViewAutoSizeColumnMode.DisplayedCells
        Grilla.Columns(COL_FRETORNO4).AutoSizeMode = DataGridViewAutoSizeColumnMode.DisplayedCells
        Grilla.Columns(COL_CLIENTE4).AutoSizeMode = DataGridViewAutoSizeColumnMode.DisplayedCells
        Grilla.Columns(COL_MOTIVO_CIERRE4).AutoSizeMode = DataGridViewAutoSizeColumnMode.DisplayedCells
        Grilla.Columns(COL_NGUIA4).AutoSizeMode = DataGridViewAutoSizeColumnMode.DisplayedCells
        Grilla.Columns(COL_TPALLET4).AutoSizeMode = DataGridViewAutoSizeColumnMode.DisplayedCells
    End Sub
    ...
    Private Sub Grilla_CellContentClick(sender As Object, e As DataGridViewCellEventArgs) Handles Grilla.CellContentClick
        Try
            ...
            ElseIf e.ColumnIndex = COL_BTNEDITAR Then
                ...
                frm.ShowDialog()
                Call carga_grilla()
            ElseIf e.ColumnIndex = COL_BTNCIERRE Then
                Dim frm As frm_selecciona_motivo_cierre_gp = New frm_selecciona_motivo_cierre_gp
                frm.cnn = _cnn
                frm.dep_num_guia = Grilla.Rows(e.RowIndex).Cells(COL_NGUIA).Value
                frm.ShowDialog()
                Call carga_grilla()
    ...
    Private Sub carga_grilla_4()
        ...
        Try
            ...
            ds = Nothing
            classEmbarque.cnn = _cnn
            If CmbCliente4.Text = "" Then
                classEmbarque.car_codigo = 0
            Else
                classEmbarque.car_codigo = CmbCliente4.SelectedValue
            End If

            classEmbarque.emb_sello = IIf(txtSello4.Text = "", "-", txtSello4.Text)
            classEmbarque.numGuia = IIf(txtGuia4.Text = "", "0", txtGuia4.Text)
            If chkDespacho.Checked = True Then
                'fecha ingreso desde
                If CStr(dtpFechaDesde4.Value.Month).Length = 1 Then
                    mesDesde = Trim("0" + CStr(dtpFechaDesde4.Value.Month))
                Else
                    mesDesde = CStr(dtpFechaDesde4.Value.Month)
                End If

                If CStr(dtpFechaDesde4.Value.Day).Length = 1 Then
                    diaDesde = Trim("0" + CStr(dtpFechaDesde4.Value.Day))
                Else
                    diaDesde = CStr(dtpFechaDesde4.Value.Day)
                End If
                classEmbarque.fecha_despacho_desde = CStr(dtpFechaDesde4.Value.Year) + mesDesde + diaDesde

                'fecha ingreso hasta
                If CStr(dtpFechaHasta4.Value.Month).Length = 1 Then
                    mesHasta = Trim("0" + CStr(dtpFechaHasta4.Value.Month))
                Else
                    mesHasta = CStr(dtpFechaHasta4.Value.Month)
                End If

                If CStr(dtpFechaHasta4.Value.Day).Length = 1 Then
                    diaHasta = Trim("0" + CStr(dtpFechaHasta4.Value.Day))
                Else
                    diaHasta = CStr(dtpFechaHasta4.Value.Day)
                End If
                classEmbarque.fecha_despacho_hasta = CStr(dtpFechaHasta4.Value.Year) + mesHasta + diaHasta
            Else
                classEmbarque.fecha_despacho_desde = "19000101"
                classEmbarque.fecha_despacho_hasta = "20501231"
            End If
            _msg = ""
            GrillaCierre.Rows.Clear()
            ds = classEmbarque.listado_guias_pallets_cerradas(_msg)
            If _msg = "" Then
                If ds.Tables(0).Rows.Count > 0 Then
                    If ds.Tables(0).Rows(Fila)("emb_sello") <> "" Then
                        Do While Fila < ds.Tables(0).Rows.Count
                            GrillaCierre.Rows.Add(ds.Tables(0).Rows(Fila)("emb_sello"),
                                            ds.Tables(0).Rows(Fila)("emb_fechaingreso"),
                                            ds.Tables(0).Rows(Fila)("emb_fechadespacho"),
                                            ds.Tables(0).Rows(Fila)("car_nombre"),
