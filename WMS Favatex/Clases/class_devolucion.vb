...
Public Class class_devolucion
    ...
    Public Function DEVOLUCION_PRODUCTO_ORDEN_CONSOLIDADO_NEW(ByRef Mensaje As String, ByVal conexion As SqlConnection) As DataSet
        Try
            Dim command As New SqlCommand("sp_devolucion_orden_consolidado_new", conexion)
            command.CommandType = CommandType.StoredProcedure

            command.Parameters.Add("@filaRegistro", SqlDbType.Int)
            command.Parameters.Add("@con_cantidad", SqlDbType.Int)

            command.Parameters(0).Value = _con_fila
            command.Parameters(1).Value = _dev_cantidad
            'command.ExecuteNonQuery()
    ...
    Public Function PICKING_GUARDA_DATOS_DEVOLUCION_DETALLE(ByRef Mensaje As String, ByVal conexion As SqlConnection) As DataSet
        Try
            ...
            command.Parameters.Add("@dev_codunico", SqlDbType.NVarChar)
    ...
    Public Function DEVOLUCIONES_DATOS_PARA_CORREO(ByRef Mensaje As String) As DataSet
        Try
            Dim conexion As New SqlConnection(_cnn)
            Dim command As New SqlCommand("sp_datos_devolucion_para_correo", conexion)
            command.CommandType = CommandType.StoredProcedure
