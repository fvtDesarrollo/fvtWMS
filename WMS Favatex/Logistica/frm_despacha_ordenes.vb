...
Public Class frm_despacha_ordenes
            ...
            Using Transaccion As New TransactionScope(TransactionScopeOption.Required, scopeOptions)
                ...
                For Each row2 As DataGridViewRow In Me.GrillaDetalle.Rows
                    If row2.Cells(colD_SELECCION).Value = True Then
                        ...
                        'Rebaja bultos del suelo
                        ds = Nothing
                        classGDescpacho.pic_codigo = _pic_codigo
                        classGDescpacho.pic_fila = row2.Cells(colD_FIL).Value
                        ds = classGDescpacho.REBAJA_BULTOS_DEL_PISO(_msgError, conexion)
                        If _msgError <> "" Then
                            If conexion.State = ConnectionState.Open Then
                                conexion.Close()
                            End If
    ...
    Private Function GUARDA_MOVIMIENTO(ByVal conexion As SqlConnection) As Boolean
        ...
        Dim filaSubProductos As Integer = 0


        GUARDA_MOVIMIENTO = False

        Try
            _Observacion = "MOVIMIENTO GENERADO A TRAVEZ DEL DESPACHO DEL PK NUMERO:  " + _pic_codigo.ToString
            ...
            '=============================================================
            If GUARDA_ENC_MOVIMINETO(conexion, 0, TIPO_MOVIMIENTO.SALIDA, GLO_BODEGA_PRODUCTO_TERMINADO, _fecha, mes, _fecha.Year, GLO_USUARIO_ACTUAL, cmbCliente.SelectedValue,
                                         COM_TIPO_DOCUMENTO.VALE_DE_SALIDA, _pic_codigo, 0, _Observacion, "-", 0, _Folio) = False Then
                Return False
                Exit Function
            End If
            ...
            Do While fila <= GrillaDetalle.Rows.Count - 1
                If CInt(GrillaDetalle.Rows(fila).Cells(colD_EXISTENTES).Value) > 0 Then
                    filaSubProductos = 0
                    _msgError = ""

                    class_productos.pro_codigo = GrillaDetalle.Rows(fila).Cells(colD_NEW_CODIGO).Value
                    ds = class_productos.PRODUCTOS_LISTADO_RELACIONADO_EN_COMBO_TRANSACCION(_msgError, conexion)
                    If _msgError = "" Then
                        If ds.Tables(0).Rows.Count > 0 Then
                            If ds.Tables(0).Rows(filaSubProductos)("pro_codigo") > 0 Then
                                Do While filaSubProductos < ds.Tables(0).Rows.Count
                                    If GUARDA_DET_MOVIMINETO(conexion, _Folio, ds.Tables(0).Rows(filaSubProductos)("pro_codigo"), (CInt(GrillaDetalle.Rows(fila).Cells(colD_EXISTENTES).Value) * ds.Tables(0).Rows(filaSubProductos)("pro_cantidad")), ds.Tables(0).Rows(filaSubProductos)("pro_cantidadbulto"), CInt(GrillaDetalle.Rows(fila).Cells(colD_EXISTENTES).Value) * ds.Tables(0).Rows(filaSubProductos)("pro_cantidadbulto")) = False Then
                                        Return False
                                        Exit Function
                                    End If

                                    If ACTUALIZA_STOCK_BODEGA(conexion, GLO_BODEGA_RECEPCION, ds.Tables(0).Rows(filaSubProductos)("pro_codigo"), TIPO_MOVIMIENTO.SALIDA, (CInt(GrillaDetalle.Rows(fila).Cells(colD_EXISTENTES).Value) * ds.Tables(0).Rows(filaSubProductos)("pro_cantidad"))) = False Then
                                        Return False
                                        Exit Function
                                    End If

                                    filaSubProductos = filaSubProductos + 1

                                Loop
                            Else
                                If GUARDA_DET_MOVIMINETO(conexion, _Folio, GrillaDetalle.Rows(fila).Cells(colD_NEW_CODIGO).Value, CInt(GrillaDetalle.Rows(fila).Cells(colD_EXISTENTES).Value), CInt(GrillaDetalle.Rows(fila).Cells(colD_NBXU).Value), CInt(GrillaDetalle.Rows(fila).Cells(colD_TBULTOS).Value)) = False Then
                                    Return False
                                    Exit Function
                                End If

                                If ACTUALIZA_STOCK_BODEGA(conexion, GLO_BODEGA_RECEPCION, GrillaDetalle.Rows(fila).Cells(colD_NEW_CODIGO).Value, TIPO_MOVIMIENTO.SALIDA, CInt(GrillaDetalle.Rows(fila).Cells(colD_EXISTENTES).Value)) = False Then
                                    Return False
                                    Exit Function
                                End If
                            End If
