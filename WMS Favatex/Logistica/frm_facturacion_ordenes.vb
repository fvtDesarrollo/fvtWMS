...
Public Class frm_facturacion_ordenes
    ...
    Private Sub CARGA_GRILLA_DETALLE()
        ...
        Try
            ...
            If txtOrdenCompra.Text = "" Or txtOrdenCompra.Text = "0" Then
                classFactura.pic_ocnumero = 0
            Else
                classFactura.pic_ocnumero = CLng(txtOrdenCompra.Text)
            End If
        ...
        If txtNumero.Text <> "" And txtNumero.Text <> "0" Then
            For Each row As DataGridViewRow In Me.GrillaDetalle.Rows
                If row.Cells(colD_SELECCION).Value = True Then
                    row.Cells(colD_NUM_FACTURA).Value = txtNumero.Text
                End If
            Next
        End If
            ...
            Using Transaccion As New TransactionScope(TransactionScopeOption.Required, scopeOptions)
                ...
                For Each row2 As DataGridViewRow In Me.GrillaDetalle.Rows
                    If row2.Cells(colD_SELECCION).Value = True Then
                        ...
                        classFactura.fac_numero = row2.Cells(colD_NUM_FACTURA).Value 'txtNumero.Text
                    ...
                    classPicking.lpi_descripcion = "Se genera factura n°: " + row2.Cells(colD_NUM_FACTURA).Value.ToString ' txtNumero.Text
    ...
    Private Sub txtBuscarFactura_Click(sender As Object, e As EventArgs) Handles txtBuscarFactura.Click
        Dim classManager As class_manager = New class_manager
        ...
        Try
            ...
            classManager.cnn = GLO_CONEXION_MANAGER
            For Each row As DataGridViewRow In Me.GrillaDetalle.Rows
                classManager.tipo = TipoDocumento
                classManager.pic_codigo = row.Cells(colD_PIC_CODIGO).Value
                classManager.pic_fila = row.Cells(colD_FIL).Value
                ds = classManager.OBTIENE_NUMERO_VENTA(_msg)
                If _msg = "" Then
                    If ds.Tables(0).Rows.Count > 0 Then
                        If ds.Tables(0).Rows(0)("numDoc") <> 0 Then
                            row.Cells(colD_NUM_FACTURA).Value = ds.Tables(0).Rows(0)("numDoc")
                            row.Cells(colD_SELECCION).Value = True
                        End If
                    End If
                    'Call PINTA_FILA_GRILLA()
                Else
