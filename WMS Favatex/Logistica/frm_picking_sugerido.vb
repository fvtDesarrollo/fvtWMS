...
Public Class frm_picking_sugerido
    ...
    Const COL_GRI_ENC_SELECCION As Integer = 0
    Const COL_GRI_ENC_CAR_CODIGO As Integer = 1
    Const COL_GRI_ENC_ORDEN_COMPRA As Integer = 2
    Const COL_GRI_ENC_FECHA_DESPACHO As Integer = 3
    Const COL_GRI_ENC_BODEGA_DESPACHO As Integer = 4
    Const COL_GRI_ENC_TOTAL As Integer = 5
    Const COL_GRI_ENC_ARCHIVO As Integer = 6
    Const COL_GRI_ENC_CANT_ENCONTRADA As Integer = 7
    Const COL_GRI_ENC_ES_DEVOLUCION As Integer = 8
    Const COL_GRI_ENC_COMUNA_RECEPTOR As Integer = 9
    Const COL_GRI_CON_OBJETADA As Integer = 10
    ...
    Const PRECIO_MANAGER As Integer = 12
    Const FECHA_DESPACHO As Integer = 13
    Const con_despacharanumero As Integer = 14
    Const con_despachara As Integer = 15
    Const con_local As Integer = 16
    Const con_localnombre As Integer = 17
    Const rut_cliente As Integer = 18
    Const nombre_cliente As Integer = 19
    Const con_observacion As Integer = 20
    Const PRO_CODIGO As Integer = 21
    Const PDD_CODIGO As Integer = 22
    Const CON_FILA As Integer = 23
    ...
    Private Sub frm_picking_sugerido_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        Me.Grilla.CurrentCell = Nothing
        Call INICIALIZA()
    End Sub

    Private Sub CONFIGURA_COLUMNAS()
        Grilla.Columns(COL_GRI_ENC_CAR_CODIGO).AutoSizeMode = DataGridViewAutoSizeColumnMode.DisplayedCells
        Grilla.Columns(COL_GRI_ENC_ORDEN_COMPRA).AutoSizeMode = DataGridViewAutoSizeColumnMode.DisplayedCells
        Grilla.Columns(COL_GRI_ENC_FECHA_DESPACHO).AutoSizeMode = DataGridViewAutoSizeColumnMode.DisplayedCells
        Grilla.Columns(COL_GRI_ENC_BODEGA_DESPACHO).AutoSizeMode = DataGridViewAutoSizeColumnMode.DisplayedCells
        Grilla.Columns(COL_GRI_ENC_TOTAL).AutoSizeMode = DataGridViewAutoSizeColumnMode.DisplayedCells
        Grilla.Columns(COL_GRI_ENC_ARCHIVO).AutoSizeMode = DataGridViewAutoSizeColumnMode.DisplayedCells
        Grilla.Columns(COL_GRI_ENC_CANT_ENCONTRADA).AutoSizeMode = DataGridViewAutoSizeColumnMode.DisplayedCells
        Grilla.Columns(COL_GRI_ENC_ES_DEVOLUCION).AutoSizeMode = DataGridViewAutoSizeColumnMode.DisplayedCells
        Grilla.Columns(COL_GRI_ENC_COMUNA_RECEPTOR).AutoSizeMode = DataGridViewAutoSizeColumnMode.DisplayedCells
    End Sub
    ...
    Private Sub CARGA_GRILLA_FECHAS()
        ...
        Try
            ...
            If txtOC.Text = "" Or txtOC.Text = "0" Then
                classSolicitud.oc_numero = 0
            Else
                classSolicitud.oc_numero = CLng(txtOC.Text)
            End If
    ...
    Private Sub IdentificaOrdenesInconcistentes()
        ...
        Try
            ...
            For Each row As DataGridViewRow In Me.Grilla.Rows
                cadenaOrdenes = cadenaOrdenes + row.Cells(COL_GRI_ENC_ORDEN_COMPRA).Value.ToString + ","
            Next
                ...
                If ds.Tables(0).Rows.Count > 0 Then
                    Do While Fila < ds.Tables(0).Rows.Count
                        If ds.Tables(0).Rows(Fila)("oc_numero") > 0 Then
                            FilaGrilla = 0
                            Do While FilaGrilla <= Grilla.Rows.Count - 1
                                If ds.Tables(0).Rows(Fila)("oc_numero") = Grilla.Rows(FilaGrilla).Cells(COL_GRI_ENC_ORDEN_COMPRA).Value Then
                                    Grilla.Rows(FilaGrilla).Cells(COL_GRI_CON_OBJETADA).Value = 1
                                    Grilla.Rows(FilaGrilla).DefaultCellStyle.BackColor = Color.IndianRed
                                    Grilla.Rows(FilaGrilla).DefaultCellStyle.ForeColor = Color.White
                                End If
        ...
        Try
            ...
            For Each row As DataGridViewRow In Me.GrillaFechas.Rows
                If row.Cells(0).Value = True Then
                    ...
                    If txtOC.Text = "" Or txtOC.Text = "0" Then
                        classSolicitud.con_ocnumero = 0
                    Else
                        classSolicitud.con_ocnumero = CLng(txtOC.Text)
                    End If
    ...
    Private Sub CARGA_ORDEN_COMPRA_DETALLE(ByVal numOC As Long, ByVal estado As Integer, ByVal fecha As Date, ByVal despachar As String)
        ...
        Try
            ...
            classSolicitud.obtiene_precion_manager = 1
                    ...
                    If ds.Tables(0).Rows(Fila)("sku_cartera") <> "" Then
                        Do While Fila < ds.Tables(0).Rows.Count
                            ...
                            GrillaDetalle.Rows.Add(False, ds.Tables(0).Rows(Fila)("car_codigo"),
                                            ...
                                            FormatNumber(ds.Tables(0).Rows(Fila)("con_preciomanager"), 0),
                            ...
                            If ds.Tables(0).Rows(Fila)("con_preciocosto") < ds.Tables(0).Rows(Fila)("con_preciomanager") Then
                                PintaPrecio = True
                            End If
    ...
    Private Sub PintaCeldaEnRojo(ByVal fila As Integer, ByVal PintaCodigo As Boolean, ByVal PintaPrecio As Boolean)
        Try
            If PintaCodigo = True Then
                GrillaDetalle.Item(CODIGO_INTERNO, fila).Style.BackColor = Color.IndianRed
                GrillaDetalle.Item(CODIGO_INTERNO, fila).Style.ForeColor = Color.White

                GrillaDetalle.Item(NOMBRE_FAVATEX, fila).Style.BackColor = Color.IndianRed
                GrillaDetalle.Item(NOMBRE_FAVATEX, fila).Style.ForeColor = Color.White
            End If

            If PintaPrecio = True Then
                GrillaDetalle.Item(PRECIO, fila).Style.BackColor = Color.IndianRed
                GrillaDetalle.Item(PRECIO, fila).Style.ForeColor = Color.White

                GrillaDetalle.Item(PRECIO_MANAGER, fila).Style.BackColor = Color.IndianRed
                GrillaDetalle.Item(PRECIO_MANAGER, fila).Style.ForeColor = Color.White
            End If
    ...
    Private Sub ToolStripButton1_Click(sender As Object, e As EventArgs) Handles ButtonGenera.Click
        ...
        For Each row As DataGridViewRow In Me.Grilla.Rows
            If row.Cells(COL_GRI_ENC_SELECCION).Value = True And row.Cells(COL_GRI_CON_OBJETADA).Value = 1 Then
                contador = contador + 1
            End If
        Next

        If contador > 0 Then
            MessageBox.Show("No es posible generar el PK ya que dentro de la selección existen ordenes objetadas", Me.Text, MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
            Exit Sub
        End If
    ...
    Private Sub GENERA_PICKING()
        ...
        Dim classSolicitud As class_solicitud_picking = New class_solicitud_picking
        Dim classLog As LogEventosR = New LogEventosR
        Dim ds As DataSet = New DataSet
        Dim _msgError As String = ""
            ...
            Using Transaccion As New TransactionScope(TransactionScopeOption.Required, scopeOptions)
                ...
                For Each row As DataGridViewRow In Me.Grilla.Rows
                    If row.Cells(0).Value = True Then
                        ...
                        classSolicitud.obtiene_precion_manager = 0
                            ...
                            If ds.Tables(0).Rows.Count > 0 Then
                                If ds.Tables(0).Rows(fila)("sku_cartera") <> "" Then
                                    Do While fila < ds.Tables(0).Rows.Count
                                        ...
                                        ds.Tables(0).Rows(fila)("dpp_fila"),
                                        ds.Tables(0).Rows(fila)("con_fila"),
                                        ds.Tables(0).Rows(fila)("con_preciomanager")))
                ...
                For Each p As estructura_detalle_picking In listaDetallePicking
                    ...
                    classPicking.precioManager = p.precioManager
    ...
    Private Sub GrillaDetalle_CellContentClick(sender As Object, e As DataGridViewCellEventArgs) Handles GrillaDetalle.CellContentClick
        If GrillaDetalle.Rows.Count > 0 Then
            Call CARGA_GRILLA_BODEGAS(GrillaDetalle.Rows(e.RowIndex).Cells(21).Value)
        End If
    ...
    Private Sub CARGACARGA_DATOS_DEJAR_PENDIENTE_CORREO()
        ...
        Try
            ...
            For Each row As DataGridViewRow In Me.Grilla.Rows
                If row.Cells(0).Value = True Then
                    corrMensaje = corrMensaje + row.Cells(2).Value.ToString + " - " + row.Cells(3).Value + "<br>"
                End If
            Next
    ...
    Private Sub ButtonEmail_Click(sender As Object, e As EventArgs) Handles ButtonEmail.Click
        ...
        contador = 0
        contadorNoObjetadas = 0
        contadorSeleccionadas = 0
        For Each row As DataGridViewRow In Me.Grilla.Rows
            If row.Cells(COL_GRI_ENC_SELECCION).Value = True Then
            ...
            'If row.Cells(COL_GRI_ENC_SELECCION).Value = True And row.Cells(COL_GRI_CON_OBJETADA).Value = 0 Then
            '    contadorNoObjetadas = contadorNoObjetadas + 1
            'End If
        Next

        If contadorSeleccionadas = 0 Then
            MessageBox.Show("Debe seleccionar a lo menos una orden de compra", Me.Text, MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
            Exit Sub
        End If

        'If contadorNoObjetadas > 0 Then
