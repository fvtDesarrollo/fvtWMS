...
Public Class frm_mant_picking
    ...
    Const col_CON_UBICACION As Integer = 25
    Const col_TIPO_DEVOLUCION_CODIGO As Integer = 26
    Const col_CONS_FILA As Integer = 27
    Const col_OBSERVACIONES2 As Integer = 28
    ...
    Private Sub frm_mant_picking_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        Else
            ...
            btnQuitaFila.Enabled = False
        End If

    End Sub

    Private Sub CONFIGURA_COLUMNAS()
        GrillaDetalle.Columns(col_SELECCION).AutoSizeMode = DataGridViewAutoSizeColumnMode.DisplayedCells
        GrillaDetalle.Columns(col_CAR_CODIGO).AutoSizeMode = DataGridViewAutoSizeColumnMode.DisplayedCells
        GrillaDetalle.Columns(col_FIL).AutoSizeMode = DataGridViewAutoSizeColumnMode.DisplayedCells
        GrillaDetalle.Columns(col_CODIGO_INTERNO).AutoSizeMode = DataGridViewAutoSizeColumnMode.DisplayedCells
        GrillaDetalle.Columns(col_NOMBRE_FAVATEX).AutoSizeMode = DataGridViewAutoSizeColumnMode.DisplayedCells
        GrillaDetalle.Columns(col_SKU_CLIENTE).AutoSizeMode = DataGridViewAutoSizeColumnMode.DisplayedCells
        GrillaDetalle.Columns(col_SKU_NOMBRE).AutoSizeMode = DataGridViewAutoSizeColumnMode.DisplayedCells
        GrillaDetalle.Columns(col_DISPONIBLE).AutoSizeMode = DataGridViewAutoSizeColumnMode.DisplayedCells
        GrillaDetalle.Columns(col_CANTIDAD).AutoSizeMode = DataGridViewAutoSizeColumnMode.DisplayedCells
        GrillaDetalle.Columns(col_NBXU).AutoSizeMode = DataGridViewAutoSizeColumnMode.DisplayedCells
        GrillaDetalle.Columns(col_TBULTOS).AutoSizeMode = DataGridViewAutoSizeColumnMode.DisplayedCells
        GrillaDetalle.Columns(col_CODIGO_INTERNO_ORIGINAL).AutoSizeMode = DataGridViewAutoSizeColumnMode.DisplayedCells
        GrillaDetalle.Columns(col_EXISTENTES).AutoSizeMode = DataGridViewAutoSizeColumnMode.DisplayedCells
        GrillaDetalle.Columns(col_OBSERVACIONES).AutoSizeMode = DataGridViewAutoSizeColumnMode.DisplayedCells
        GrillaDetalle.Columns(col_NEW_CODIGO).AutoSizeMode = DataGridViewAutoSizeColumnMode.DisplayedCells
        GrillaDetalle.Columns(col_COD_INT_ORI).AutoSizeMode = DataGridViewAutoSizeColumnMode.DisplayedCells
        GrillaDetalle.Columns(col_NOMBRE_ORI).AutoSizeMode = DataGridViewAutoSizeColumnMode.DisplayedCells
        GrillaDetalle.Columns(col_NBO).AutoSizeMode = DataGridViewAutoSizeColumnMode.DisplayedCells
        GrillaDetalle.Columns(col_PIC_CODIGO).AutoSizeMode = DataGridViewAutoSizeColumnMode.DisplayedCells
        GrillaDetalle.Columns(col_NUEVA).AutoSizeMode = DataGridViewAutoSizeColumnMode.DisplayedCells
        GrillaDetalle.Columns(col_PRECIO_UNITARIO).AutoSizeMode = DataGridViewAutoSizeColumnMode.DisplayedCells
        GrillaDetalle.Columns(col_OC_ORIGEN).AutoSizeMode = DataGridViewAutoSizeColumnMode.DisplayedCells
        GrillaDetalle.Columns(col_FECHA_OC).AutoSizeMode = DataGridViewAutoSizeColumnMode.DisplayedCells
        GrillaDetalle.Columns(col_COD_UNICO).AutoSizeMode = DataGridViewAutoSizeColumnMode.DisplayedCells
        GrillaDetalle.Columns(col_FILA_DEVULETA).AutoSizeMode = DataGridViewAutoSizeColumnMode.DisplayedCells
        GrillaDetalle.Columns(col_CON_UBICACION).AutoSizeMode = DataGridViewAutoSizeColumnMode.DisplayedCells
        GrillaDetalle.Columns(col_OBSERVACIONES2).AutoSizeMode = DataGridViewAutoSizeColumnMode.DisplayedCells
    End Sub
                    ...
                    If ds.Tables(0).Rows(Fila)("sku_cartera_nombre") <> "" Then
                        Do While Fila < ds.Tables(0).Rows.Count
                            GrillaDetalle.Rows.Add(False, 0,
                                                   ...
                                                   ds.Tables(0).Rows(Fila)("tdp_codigo"),
                                                   ds.Tables(0).Rows(Fila)("con_fila"),
                                                   ds.Tables(0).Rows(Fila)("observaciones2"))
    ...
    Private Sub GrillaDetalle_CellContentClick(sender As Object, e As DataGridViewCellEventArgs) Handles GrillaDetalle.CellContentClick
        ...
        'If e.ColumnIndex = Me.GrillaDetalle.Columns.Item(col_SELECCION).Index Then
        '    Dim chkCell As DataGridViewCheckBoxCell = Me.GrillaDetalle.Rows(e.RowIndex).Cells(col_SELECCION)
        '    chkCell.Value = Not chkCell.Value
        'End If

        'If GrillaDetalle.Rows(e.RowIndex).Cells(col_TIPO_DEVOLUCION_CODIGO).Value > 0 Then
        '    Dim chkCell As DataGridViewCheckBoxCell = Me.GrillaDetalle.Rows(e.RowIndex).Cells(col_SELECCION)
        '    chkCell.Value = True
        'Else
        '    Dim chkCell As DataGridViewCheckBoxCell = Me.GrillaDetalle.Rows(e.RowIndex).Cells(col_SELECCION)
    ...
    Private Sub EliminarProductoDelPickingToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles EliminarProductoDelPickingToolStripMenuItem.Click
        ...
        Try
            ...
            Using Transaccion As New TransactionScope(TransactionScopeOption.Required, scopeOptions)
                ...
                ds = classDev.DEVOLUCION_PRODUCTO_ORDEN_CONSOLIDADO_NEW(_msgError, conexion)
                If _msgError = "" Then
                    If ds.Tables.Count > 0 Then
                        If ds.Tables(0).Rows(0)("CODIGO") < 0 Then
                            If conexion.State <> ConnectionState.Closed Then
    ...
    Private Sub BtnPendiente_Click(sender As Object, e As EventArgs) Handles BtnPendiente.Click
        ...
        Try
            ...
            For Each row As DataGridViewRow In Me.GrillaDetalle.Rows
                If row.Cells(col_SELECCION).Value = True And row.Cells(col_TIPO_DEVOLUCION_CODIGO).Value > 0 Then
                    row.Cells(col_SELECCION).Value = False
                End If
            Next
            ...
            If contador = 0 Then
                MessageBox.Show("Debe seleccionar a lo menos un item", Me.Text, MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
                Exit Sub
            End If

            respuesta = MessageBox.Show("¿Esta seguro(a) de quitar la(s) fila(s) seleccionada(s)", Me.Text, MessageBoxButtons.YesNo, MessageBoxIcon.Question)
            If respuesta = vbNo Then
                Exit Sub
            End If
            ...
            Using Transaccion As New TransactionScope(TransactionScopeOption.Required, scopeOptions)
                ...
                If GLO_TIPO_DEVOLUCION_PICKING_CODIGO <> 0 Then
                    For Each row As DataGridViewRow In Me.GrillaDetalle.Rows
                        If row.Cells(col_SELECCION).Value = True Then
                            codigo = row.Cells(col_NEW_CODIGO).Value
                            row.Cells(col_FILA_DEVULETA).Value = True
                            row.Cells(col_OBSERVACIONES).Value = GLO_TIPO_DEVOLUCION_PICKING_NOMBRE
                            row.Cells(col_TIPO_DEVOLUCION_CODIGO).Value = GLO_TIPO_DEVOLUCION_PICKING_CODIGO
                            row.Cells(col_OBSERVACIONES2).Value = GLO_TIPO_DEVOLUCION_PICKING_OBSERVACION


                            classDev.cnn = _cnn
                            classDev.pic_codigo = CLng(txtNumero.Text)
                            classDev.con_fila = row.Cells(col_CONS_FILA).Value
                            classDev.dev_cantidad = row.Cells(col_CANTIDAD).Value
                            ds = classDev.DEVOLUCION_PRODUCTO_ORDEN_CONSOLIDADO_NEW(_msgError, conexion)
                            If _msgError = "" Then
                                If ds.Tables.Count > 0 Then
                                    If ds.Tables(0).Rows(0)("CODIGO") < 0 Then
                            ...
                            classPicking.pic_codigo = CLng(txtNumero.Text)
                            classPicking.pic_fila = row.Cells(col_FIL).Value
                            classPicking.observaciones = GLO_TIPO_DEVOLUCION_PICKING_NOMBRE
                            classPicking.tdp_codigo = row.Cells(col_TIPO_DEVOLUCION_CODIGO).Value
                            classPicking.observacion2 = row.Cells(col_OBSERVACIONES2).Value
                            ds = classPicking.PICKING_QUITA_FILA(_msgError, conexion)
                            If _msgError = "" Then
                                If ds.Tables.Count > 0 Then
                                    If ds.Tables(0).Rows(0)("CODIGO") < 0 Then
                            ...
                            classPicking.pic_codigo = _pic_codigo
                            classPicking.usu_codigo = GLO_USUARIO_ACTUAL
                            classPicking.lpi_descripcion = "Se deja disponible fila " + row.Cells(col_FIL).Value.ToString + "del picking, para un reproceso"
                            ds = classPicking.PICKING_GUARDA_LOG(_msgError, conexion)
                            If _msgError <> "" Then
                                Me.Cursor = Cursors.Default
                                If conexion.State = ConnectionState.Open Then
                ...
                If GLO_TIPO_DEVOLUCION_PICKING_CODIGO <> 0 Then
                    Call CARGACARGA_DATOS_DEJAR_PENDIENTE_CORREO(GLO_TIPO_DEVOLUCION_PICKING_NOMBRE, GLO_TIPO_DEVOLUCION_PICKING_OBSERVACION, 1)
                End If
    ...
    Private Sub btnQuitaFila_Click(sender As Object, e As EventArgs) Handles btnQuitaFila.Click
        Dim contador As Integer = 0
        Dim classDev As class_devolucion = New class_devolucion
        Dim classPicking As class_picking = New class_picking
        Dim codigo As Integer = 0
        ...
        Try
            ...
            Using Transaccion As New TransactionScope(TransactionScopeOption.Required, scopeOptions)
                ...
                If GLO_TIPO_DEVOLUCION_PICKING_CODIGO <> 0 Then
                    For Each row As DataGridViewRow In Me.GrillaDetalle.Rows
                        If row.Cells(col_SELECCION).Value = True Then
                            ...
                            ds = Nothing
                            classPicking.pic_codigo = CLng(txtNumero.Text)
                            classPicking.pic_fila = row.Cells(col_FIL).Value 'GrillaDetalle.Rows(_fila).Cells(col_FIL).Value
                            classPicking.observaciones = "* " + row.Cells(col_OBSERVACIONES).Value 'GLO_TIPO_DEVOLUCION_PICKING_NOMBRE
                            classPicking.tdp_codigo = row.Cells(col_TIPO_DEVOLUCION_CODIGO).Value 'GLO_TIPO_DEVOLUCION_PICKING_CODIGO
                            classPicking.observacion2 = row.Cells(col_OBSERVACIONES2).Value
                            ds = classPicking.PICKING_QUITA_FILA(_msgError, conexion)
                            If _msgError = "" Then
                                If ds.Tables.Count > 0 Then
                            ...
                            classPicking.pic_codigo = _pic_codigo
                            classPicking.usu_codigo = GLO_USUARIO_ACTUAL
                            classPicking.lpi_descripcion = "Se Anula fila " + row.Cells(col_FIL).Value.ToString + " del picking, por: " + GLO_TIPO_DEVOLUCION_PICKING_NOMBRE
                            ds = classPicking.PICKING_GUARDA_LOG(_msgError, conexion)
                            If _msgError <> "" Then
                                Me.Cursor = Cursors.Default
                                If conexion.State = ConnectionState.Open Then
                ...
                If GLO_TIPO_DEVOLUCION_PICKING_CODIGO <> 0 Then
                    Call CARGACARGA_DATOS_DEJAR_PENDIENTE_CORREO(GLO_TIPO_DEVOLUCION_PICKING_NOMBRE, GLO_TIPO_DEVOLUCION_PICKING_OBSERVACION, 2)
                End If
    ...
    Private Sub CARGACARGA_DATOS_DEJAR_PENDIENTE_CORREO(ByVal Motivo As String, ByVal Motivo2 As String, ByVal accion As Integer)
        Dim classUsuario As class_usuario = New class_usuario
        ...
        Try
            ...
            If accion = 1 Then
                corrAsunto = "Item(s) PENDIENTE(S) PIKING Nº: " + txtNumero.Text + " CLIENTE: " + cmbCliente.Text
                corrMensaje = "Favor considerar que del PK " + txtNumero.Text + " " + cmbCliente.Text + " se dejan pendientes los siguientes c&oacute;digos. " + Motivo + "  " + Motivo2 + "
	                      <br><br>"
            Else
                corrAsunto = "Item(s) ANULADO(S) PIKING Nº: " + txtNumero.Text + " CLIENTE: " + cmbCliente.Text
                corrMensaje = "Favor considerar que del PK " + txtNumero.Text + " " + cmbCliente.Text + " se anulan los siguientes c&oacute;digos. " + Motivo + "  " + Motivo2 + "
	                      <br><br>"
            End If
            ...
            For Each row As DataGridViewRow In Me.GrillaDetalle.Rows
                If row.Cells(col_SELECCION).Value = True Then
                    corrMensaje = corrMensaje + row.Cells(col_CODIGO_INTERNO).Value + " " + row.Cells(col_NOMBRE_FAVATEX).Value + " x " + row.Cells(col_CANTIDAD).Value + "UNIDAD(ES)" + "<br>"
                End If
            Next

            If accion = 1 Then
                corrMensaje = corrMensaje + "<br><br> Favor revisar reemplazo o alternativa con comercial para ofrecer al cliente. <br><br>Gracias "
            Else
